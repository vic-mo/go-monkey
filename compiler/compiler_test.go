...
func TestGlobalLetStatements(t *testing.T) {
	tests := []compilerTestCase{
		{input: "let one=1;let two=2;", expectedConstants: []interface{}{1, 2}, expectedInstructions: []code.Instructions{
			code.Make(code.OpConstant, 0),
			code.Make(code.OpSetGlobal, 0),
			code.Make(code.OpConstant, 1),
			code.Make(code.OpSetGlobal, 1),
		}},
		{input: "let one=1;one;", expectedConstants: []interface{}{1}, expectedInstructions: []code.Instructions{
			code.Make(code.OpConstant, 0),
			code.Make(code.OpSetGlobal, 0),
...
func TestCompilerScopes(t *testing.T) {
	...
	if last.Opcode != code.OpSub {
		t.Errorf("last instruction op code wrong. got=%d, want=%d", last.Opcode, code.OpSub)
	}
	...
	if last.Opcode != code.OpAdd {
		t.Errorf("last instruction op code wrong. got=%d, want=%d", last.Opcode, code.OpAdd)
	}
	previous := compiler.scopes[compiler.scopeIndex].previousInstruction
	if previous.Opcode != code.OpMul {
		t.Errorf("previous instruction op code wrong. got=%d, want=%d", previous.Opcode, code.OpMul)
	}
}
